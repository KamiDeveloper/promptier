...
import { adaptToModel } from '@/lib/services/aiService'
...
export async function POST(req: NextRequest) {
  ...
  const targetModel = (body as Record<string, unknown>)?.targetModel
  ...
  if (typeof targetModel !== 'string' || targetModel.trim().length === 0) {
    return NextResponse.json({ error: 'targetModel requerido' }, { status: 422 })
  }
  ...
  try {
    return NextResponse.json(await adaptToModel({ userId }, content, targetModel))
  } catch (err) {
