// POST /api/ai/variations — generate exactly 3 prompt variations
import { type NextRequest, NextResponse } from 'next/server'
import { requireAuth } from '@/lib/auth/requireAuth'
import { generateVariations } from '@/lib/services/aiService'
...
export async function POST(req: NextRequest) {
  ...
  try {
    const result = await generateVariations({ userId }, content)
